...
combines **Stripe** and **PayPal** (revenue, including MRR for the EDGE
...
4. `npm test` runs the pure-logic unit tests (period math, QuickBooks report
   parsing, Stripe multi-account parsing) — no network calls, no credentials
   required.
...
- [ ] **PayPal** — pending build. Revenue totals will use PayPal's
      Transaction Search API (`GET /v1/reporting/transactions`), which is
      well-documented and supports historical date-range queries. **MRR is a
      known limitation**: PayPal's Subscriptions API has no bulk
      "list subscriptions" endpoint (confirmed against PayPal's official
      OpenAPI spec) — only get-one-by-ID. PayPal MRR will be approximated
      from trailing subscription-tagged transactions rather than a true
...
Real (not approximated) MRR from Stripe requires knowing which Price/Product
ID corresponds to the EDGE subscription — Stripe's `list subscriptions` API
can filter by `price`/`status=active`, and each active subscription's amount
(normalized to monthly, e.g. dividing an annual price by 12) sums to true
MRR. This is pending those identifiers.
