...
// changing. Also doesn't assume which tab the data lives on -- it tries
// every tab in the workbook and keeps the first one containing the
// expected rows, since a plain range with no sheet name silently reads
// whichever tab is first by index (confirmed as the actual failure mode
// in production: the labels matched fine once read from the right tab).
// If getCashFlowProjection() throws "missing an expected row" on EVERY
// tab, the labels below need adjusting to match the sheet's actual text.
...
async function getCashFlowProjection() {
  ...
  // A plain range like "A1:R80" with no sheet name reads whichever tab
  // happens to be first by index -- if the workbook has more than one tab
  // (a cover sheet, a notes tab, etc. ahead of the actual model), that's
  // silently the wrong data. Look up every tab's title and try each one in
  // order, keeping the first that actually contains the expected rows.
  const meta = await sheets.spreadsheets.get({ spreadsheetId, fields: 'sheets.properties.title' });
  const sheetTitles = (meta.data.sheets || []).map((s) => s.properties.title);
  if (!sheetTitles.length) {
    throw new Error('Cash flow spreadsheet has no tabs');
  }
  ...
  for (const title of sheetTitles) {
    try {
      const resp = await sheets.spreadsheets.values.get({
        spreadsheetId,
        range: `'${title.replace(/'/g, "''")}'!A1:R80`,
        valueRenderOption: 'UNFORMATTED_VALUE',
      });
      return parseProjectionRows(resp.data.values || []);
    } catch (err) {
      lastError = err;
    }
  }
