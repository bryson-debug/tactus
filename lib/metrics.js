...
async function buildSummary({ period = 'this_month', customRange } = {}) {
  ...
  const [stripe, paypal, quickbooks, quickbooksInvoices, stripeMrr, paypalMrr] = await Promise.all([
    ...
    settle(quickbooksClient.getPaidInvoicesSummary({ startDate, endDate })),
