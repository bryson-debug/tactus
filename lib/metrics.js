...
const cashflowSheetClient = require('./cashflow-sheet-client');
const flodeskClient = require('./flodesk-client');
...
async function buildSummary({ period = 'this_month', customRange } = {}) {
  ...
  const [
    ...
    settle(quickbooksClient.getPaymentsReceivedSummary({ startDate, endDate })),
    ...
    // Not period-scoped -- the sheet has its own fixed 14-month range,
    // shown as-is regardless of the dashboard's selected period.
    settle(cashflowSheetClient.getCashFlowProjection()),
    settle(flodeskClient.getSubscriberGrowth({ startDate, endDate })),
    settle(flodeskClient.getSubscriberChurn({ startDate, endDate })),
  ]);
  ...
  return {
    ...
    cashFlowProjection,
    flodesk: { growth: flodeskGrowth, churn: flodeskChurn },
  };
}
