...
// buildSummary() in lib/metrics.js fetches the P&L report and payments-received
// summary concurrently, and both call getValidAccessToken() -- if the stored
// token is near its ~1hr expiry, both would otherwise independently call
// refreshAccessToken() with the same refresh_token. Intuit's refresh tokens
// are single-use, so the second call fails with a 400 (confirmed as the exact
// cause of a real "QuickBooks token request failed: 400" error in production).
// Sharing one in-flight refresh across concurrent callers avoids that race.
...
async function getValidAccessToken() {
  ...
  if (!inFlightRefresh) {
    inFlightRefresh = refreshAccessToken(stored.refresh_token, stored.realm_id).finally(() => {
...
// Queries the Payment entity directly rather than inferring "paid" from
// Invoice.Balance -- Payment.TxnDate is the actual date the money was
// received (confirmed against Intuit's Payment entity docs), which
// Invoice doesn't expose. A Payment record only exists once money has
// actually come in, so no extra "is it paid" filter is needed the way
// Invoice needed Balance = '0'. Represents revenue collected outside
// Stripe/PayPal (e.g. check, bank transfer) -- kept separate from those so
...
function sumTransactionAmounts(records) {
  ...
  for (const record of records) {
    grossCents += Math.round((Number(record.TotalAmt) || 0) * 100);
    currency = currency || record.CurrencyRef?.value;
    count += 1;
  }
  return { grossCents, currency, count };
}

async function getPaymentsReceivedSummary({ startDate, endDate }) {
  const { accessToken, realmId } = await getValidAccessToken();
  ...
  for (;;) {
    ...
    const body = await resp.json();
    if (!resp.ok) {
      const err = new Error(`QuickBooks Payment query failed: ${resp.status}`);
      err.body = body;
      throw err;
    }

    const payments = body?.QueryResponse?.Payment || [];
    const page = sumTransactionAmounts(payments);
    grossCents += page.grossCents;
    currency = currency || page.currency;
    paymentCount += page.count;

    if (payments.length < MAX_RESULTS) break;
    startPosition += MAX_RESULTS;
  }

  return { source: 'quickbooks_payments', currency: (currency || 'usd').toLowerCase(), grossCents, paymentCount };
}
